...
class MaximumVertexCover(Problem):
    ...
    def __init__(self, k, n_vertex, n_edges, vertex, edges, initial=None):
        self.k = k
    ...
    def build_init(self):
        cover = list(range(self.k))
        not_cover = list(range(self.k,self.n_vertex))
        return State(cover, not_cover)
    ...
    def successor(self, state):
        for u in state.cover:
            for v in state.not_cover:
                yield (None,State([x for x in state.cover if x != u]+[v],[x for x in state.not_cover if x != v]+[u]))
